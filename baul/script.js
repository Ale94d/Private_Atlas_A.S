...
const emptyTravel = document.getElementById("emptyTravel");
...
function loadTravels() {
    ...
    const travels =
        JSON.parse(
            localStorage.getItem("privateAtlasTravels")
        ) || [];
...
function createTravelCard(travel) {
    ...
    card.innerHTML = `
        ...
        <div class="travel-card-info">
            <h3>${escapeHTML(travel.name || travel.title || "Viaje")}</h3>

            <span>
                <i data-lucide="map"></i>
                ${escapeHTML(travel.destination || "Destino no especificado")}
            </span>
        </div>

        <div class="travel-card-date">
            <i data-lucide="calendar"></i>
            ${escapeHTML(travel.date || "Sin fecha")}
        </div>
